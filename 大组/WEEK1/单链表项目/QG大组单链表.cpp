...
void inputdata(LinkedList*L) //定义录入data函数 尾插法 
{
	...
	printf("请输入data：");
    int result = scanf("%d", &fresh->data);//尝试读取整数

    //检查输入是否为整数
    if (result!= 1) 
	{
        printf("输入错误，请输入整数！\n");
        free(fresh); // 释放动态分配的内存
        while (getchar()!='\n'); // 清空输入缓冲区
        return;
    }
	if(L->head==NULL) 
...
void insertlist(LinkedList* L)//遍历链表，新结点插入到上一个结点的尾部，下一个结点的头部 
{
    Node* fresh=(Node*)malloc(sizeof(Node)); //动态分配内存，创建新结点fresh
    if (fresh==NULL) 
	{
    ...
    while(1)
	{
		printf("请输入要插入的整数data");
		int result=scanf("%d",&fresh->data);
		// 检查输入是否为整数
        if(result==1) 
		{
            break;//输入合法，退出循环
		...
		{ 
       		...
       		switch (chose) 
			{
            	...
            	default:printf("您的输入有误，请重新输入！！！\n"); 
						system("pause"); // 暂停程序，用户查看操作结果 ,接着进行清屏操作 
			}
  		 }
  	}
